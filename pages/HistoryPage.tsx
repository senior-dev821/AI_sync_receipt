...
export const HistoryPage: React.FC = () => {
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-10 pb-32 text-slate-100">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
        <div className="bg-[#0b1020] p-8 rounded-3xl shadow-xl border border-slate-800/70 relative overflow-hidden group">
        ...
        <div className="bg-[#0f1522] p-8 rounded-3xl shadow-lg border border-slate-800/70">
          <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Active Receipts</p>
          <h4 className="text-3xl font-black text-slate-100">{total}</h4>
        </div>
        <div className="bg-[#0f1522] p-8 rounded-3xl shadow-lg border border-slate-800/70">
      ...
      <div className="mb-10 flex flex-col md:flex-row gap-4">
        ...
        <select
          className="px-4 py-3.5 bg-[#0f1522] border border-slate-800/70 rounded-2xl text-sm font-bold uppercase tracking-wider text-slate-300"
          value={statusFilter}
          onChange={(e) => {
        ...
        <button
          ...
          className="flex items-center justify-center gap-2 px-6 py-3.5 rounded-2xl transition-all bg-gradient-to-r from-cyan-400 via-blue-500 to-fuchsia-500 hover:from-cyan-300 hover:to-fuchsia-400 text-white shadow-lg shadow-cyan-500/30"
        >
          <span className="material-icons text-white text-xl">download</span>
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          <h2 className="text-xl font-bold text-slate-100 flex items-center gap-2 uppercase tracking-tight">
      ...
      <div className="space-y-4">
        {records.map((record) => (
          <div key={record.id} className="group bg-[#0f1522] border border-slate-800/70 rounded-2xl p-5 hover:border-primary/30 hover:shadow-xl transition-all flex flex-col md:flex-row md:items-center justify-between gap-5">
            <div className="flex items-center gap-4">
              <div className="w-14 h-14 bg-[#0b1020] rounded-xl flex items-center justify-center group-hover:bg-primary/5 transition-colors border border-slate-800/70 overflow-hidden">
              ...
              <div>
                <h3 className="font-bold text-slate-100 text-lg leading-tight">{record.vendor}</h3>
            ...
            <div className="flex items-center justify-between md:justify-end gap-6 md:gap-12 border-t md:border-t-0 pt-4 md:pt-0 border-slate-800/70">
              <div className="text-left md:text-right">
                <p className="text-xl font-black text-slate-100 tracking-tighter">${record.amount.toLocaleString(undefined, { minimumFractionDigits: 2 })}</p>
              ...
              <div className="flex items-center gap-4">
                <span className={`inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest border ${
                  record.status === ExtractionStatus.VERIFIED ? 'bg-emerald-900/30 text-emerald-300 border-emerald-800/70' : 
                  record.status === ExtractionStatus.FLAGGED ? 'bg-amber-900/30 text-amber-300 border-amber-800/70' :
                ...
                <button
                  ...
                  className="p-2 text-slate-400 hover:text-rose-400 transition-colors hover:bg-[#12192b] rounded-lg"
        ...
        {records.length === 0 && (
          <div className="text-center py-24 bg-[#0f1522] border border-dashed border-slate-800/70 rounded-3xl">
             <span className="material-icons text-5xl text-slate-500 mb-4">inventory_2</span>
      ...
      <div className="fixed bottom-10 right-8 z-50 flex items-center gap-3">
        <button 
          onClick={() => navigate('/')}
          className="h-16 px-8 bg-gradient-to-r from-cyan-400 via-blue-500 to-fuchsia-500 hover:from-cyan-300 hover:to-fuchsia-400 text-white rounded-2xl shadow-2xl shadow-cyan-500/40 flex items-center justify-center gap-3 hover:-translate-y-1 active:scale-95 transition-all group"
        >
